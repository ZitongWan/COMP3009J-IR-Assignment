...
class QueryProcessor:
    ...
    def load_index_file(self, index_file):
        ...
        print("Loading the Large Index file...")
        ...
        with open(self.queries_file, 'r', encoding='utf8') as q, open(self.output_file, 'w', encoding='utf8') as o:
            ...
            for line in q:
                ...
                query_results = sorted(self.query_processor.process_query(query).items(), key=lambda x: x[1],
                                       reverse=True)[:30]  # Return 30 documents
                end_time = time.time()
                duration = end_time - start_time
                total_time += duration
    ...
    def interactive_mode(self):
        ...
        while True:
            ...
            if query.lower() != "exit":
                ...
                if query_results:
                    print(f"Query: {query}")
                    print(f"{'Rank':<8}{'Doc ID':<24}{'Score':<14}{'StudentID':<8}")
                    for rank, (doc_id, score) in enumerate(query_results, start=1):
                        print(f"{rank:<8}{doc_id:<24}{score:<14.6f}{'22207296':<8}")
                    print()
                else:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Query_large_corpus")
    parser.add_argument('-m', '--mode', type=str, choices=['interactive', 'automatic'], required=True, help="Mode of query...")
    parser.add_argument('-p', '--path', type=str, required=True, help="Path to the large corpus directory...")

    args = parser.parse_args()

    index_path = os.path.join(args.path, "22207296-large.index.json")
    ...
    if not os.path.exists(index_path):
        ...
        output_file = os.path.join(os.getcwd(), "22207296-large.results")
