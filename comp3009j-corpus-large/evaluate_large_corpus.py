...
class Evaluator:
    ...
    def load_qrels(self, file_path):
        ...
        # eg: {'701': {'GX000-48-10208090': 0, 'GX004-05-12123594': 0, 'GX016-21-5150039': 1, 'GX025-72-6112588': 2}, '702': {...}, ...}
    ...
    def load_results(self, file_path):
        ...
        # eg: {'701': [{'doc_id': 'GX232-43-0102505', 'rank': 1, 'score': 23.1219}, {'doc_id': 'GX255-56-12408598', 'rank': 2, 'score': 21.6856}, ...], '702': [{...}]}
    ...
    def get_rel_ret(self, rel, ret, query_id):
        ...
        # eg: for query_id = xxx:
        # The target rel_rets: {1: {'doc_id': 'GX232-43-0102505', 'rank': 1, 'score': 23.1219, 'relevance': 2}, 2: {'doc_id': 'GX255-56-12408598', 'rank': 2, 'score': 21.6856, 'relevance': 1, ...}
    ...
    def bpref(self, rel, ret):
        ...
        for query_id in ret:
            ...
            for doc in rel_rets.values():
                # bpref = (1 - min(non_relevant_nums(before this doc), rel_doc_nums) / rel_doc_nums / rel_doc_nums
                # The min function is used to ensure the number of non_relevant documents ranked higher does not exceed the total number of relevant documents, which would result in a negative bpref.
                if doc['relevance'] > 0:
                    bprefs += (1 - min(non_relevant_nums, rel_doc_nums) / rel_doc_nums) / rel_doc_nums
                elif doc['relevance'] == 0:
                    non_relevant_nums += 1
        return bprefs / len(ret) if len(ret) > 0 else 0
    ...
    def evaluate(self):
        ...
        bpref = self.bpref(self.rel, self.ret)
        ...
        print(f"{'Bpref:':<12} {bpref:.4f}")
        print(f"Evaluation cost {duration} s...")
        print()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Evaluate_large_corpus")
    parser.add_argument("-p", "--path", type=str, help="Path to the large results file...")

    args = parser.parse_args()

    quels_path = os.path.join(args.path, 'files', 'qrels.txt')
    results_path = os.path.join(os.getcwd(), '22207296-large.results')

    if not os.path.exists(quels_path):
        print("Large qrels file does not exist...")
    elif not os.path.exists(results_path):
        print("Large results file does not exist...")
    else:
        evaluator = Evaluator(quels_path, results_path)
        evaluator.evaluate()
